...
def similiar_chunks_indexes(n_values, n_chunks) -> Generator[Tuple[int, int], None, None]:
    ...
    for i in range(0, n_values, chunk_size):
        if i + chunk_size >= n_values - 2:  # Avoid batches of size 1
...
def split_similar_chunks(
    vector: Union[list, Tensor], n_chunks: int
...
def split_kwargs_in_chunks(
    kwargs: Dict[str, Union[list, Tensor]], n_chunks: int
...
def split_args_in_chunks(
    args: Tuple[Union[list, Tensor]], n_chunks: int
) -> Generator[Union[list, Tensor], None, None]:
